...
// Usage:
//         AudioRecorder.exe "c:\\wavdest.ax"
// C# usage
/*
...
extern CAudioCapApp theApp; // for referncing the application theApp

// ================================================== by Jing ===============================================
CString  CurrentExecutionFilePath;
void RegistryCommon(CString strOP){
	// Step 1: Read the WavDest.ax's full directory
	if(theApp.m_lpCmdLine[0]==_T('\0')){
		TCHAR *Temp=new TCHAR[512];
		GetCurrentDirectory(512,Temp);// If user no specified, using current directory.
		CurrentExecutionFilePath=Temp;
		delete[] Temp;
	}else{
		CurrentExecutionFilePath=theApp.m_lpCmdLine; // speified user defined path
		//MessageBox(NULL,CurrentExecutionFilePath,_T("CurrentExecutionFilePath"),MB_OK);
	}

	// Step 2: Setup the current wavdest file location
	CString strFullRegWavDestFilter;
	strFullRegWavDestFilter.Format(_T("\"%s%s\""),CurrentExecutionFilePath,_T("\\wavdest.ax"));

	// Step 3: Create a registry command
	CString strCommand;
	strCommand.Format(_T("%s %s"),strOP,strFullRegWavDestFilter);

	// Step 4: Launch the command
	// MessageBox(strCommand,_T("Command"),MB_OK);
	ShellExecute(NULL, _T("open"),_T("regsvr32.exe"), strCommand.GetBuffer(), NULL, SW_SHOWNORMAL);
}
void RegistryWavDst(){
	RegistryCommon(_T("/s "));
}
void unRegistryWavDst(){
	RegistryCommon(_T("/s /u"));
}
...
void CAudioCapDlg::SetDefaults()
{
	...
	m_CheckWriteFile.SetCheck(TRUE);
...
void CAudioCapDlg::Say(TCHAR *szMsg) 
{
	...
	CString strTitle;
	strTitle.Format(_T("%s : %s"),_T("Audio Recorder"),szMsg);
