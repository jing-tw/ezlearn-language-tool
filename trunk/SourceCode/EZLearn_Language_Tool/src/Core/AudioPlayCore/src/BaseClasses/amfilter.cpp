    ...
    const CMediaType *pmt)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (i == (int)m_bTryMyTypesFirst) {
            hr = pReceivePin->EnumMediaTypes(&pEnumMediaTypes); // 列舉接受 Pin 可以接受的 media type
        } else {
            hr = EnumMediaTypes(&pEnumMediaTypes); // 列舉自己可以接受的 media type 
	...
	/*
	 ...
	 HRESULT hr = EnumMediaTypes(&pEnumMediaTypes);
	 if (SUCCEEDED(hr)) {
            ASSERT(pEnumMediaTypes);
            hr = TryMediaTypes(pReceivePin,pmt,pEnumMediaTypes);
            pEnumMediaTypes->Release();
